...
def index():
    ...
    return render_template('pages/index.html', categories = categories, latest_items = latest_items)
...
def createCatalog():
    ...
    if request.method == 'POST':
        ...
        if len(errors) == 0:
            ...
            category = Category(name = form_name, description = form_description, user = user, slug = slugify(form_name))
...
def deleteCatalog(catalog_slug):
    ...
    if len(catalog.items) > 0:
        flash('Cant delete %s because it is not empty!' % catalog.name, 'alert-danger')
...
def createCatalogItem(catalog):
    ...
    if request.method == 'POST':
        ...
        if len(errors) > 0:
            ...
            return render_template('pages/create_item.html', categories=categories, catalog=catalog, errors=errors, name=form_name, description=form_description)
        else:
            user = User.query.filter_by(id = current_user.get_id()).one()
            item = Item(name = form_name, description = form_description, category = category, user = user)
            db.session.add(item)
            db.session.commit()
            flash('Item successfuly created', 'alert-info')
            return redirect(url_for('catalogItems', slug=category.slug))
    return render_template('pages/create_item.html', categories=categories, catalog=catalog, errors=errors)
...
def deleteItem(item_id):
    ...
    if item.user != current_user:
        flash('You don\'t have the permissions to delete %s' % item.name, 'alert-danger')
        return redirect(url_for('index'))
...
def editItem(item_id):
    ...
    if item.user != current_user:
        flash('You don\'t have the permissions to edit %s' % item.name, 'alert-danger')
        return redirect(url_for('index'))
    ...
    if request.method == 'POST':
        ...
        if len(errors) > 0:
            if form_category:
                catalog = form_category
            return render_template('pages/edit_item.html', categories=categories, errors=errors, item=item)
        else:
            item.name = form_name;
